...
class PlotterApp:
    def __init__(self, root, results):
        ...
        self.patient_ids = list(results['SUBJECT_ID'].unique())
        ...
        self.prev_button = tk.Button(self.stay_button_frame, text="Previous", command=self.prev_stay)
        self.prev_button.pack(side=tk.LEFT, padx=5)

        self.next_button = tk.Button(self.stay_button_frame, text="Next", command=self.next_stay)
    ...
    def search_patient(self):
        ...
        if not pid.isdigit() or int(pid) not in self.patient_ids:
            messagebox.showerror("Invalid ID", "The patient ID is invalid. Please try again.")
            return

        self.current_patient_index = self.patient_ids.index(int(pid))
        self.update_stays_for_current_patient()
        self.plot_stay()
